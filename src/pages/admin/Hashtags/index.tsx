...
import { hashtagSlice } from "@/stores/reducers/hashtags/slice";
import {
    createHashtag,
    deleteHashtag,
    fetchHashtags,
} from "@/stores/reducers/hashtags/actions";
...
function Hashtags() {
    ...
    const { hashtags, status, error, isCreated, isDeleted } = useAppSelector(
        (state) => state.hashtag
    );
    const hashtagActions = hashtagSlice.actions;
    const { setErrorToast } = errorToastSlice.actions;
    const dispatch = useAppDispatch();

    const onDelete = async () => {
        await dispatch(deleteHashtag(String(hashtagData?.id)));
    };
    const onCreate = async (hashtagData: HashtagCreateType) => {
        await dispatch(createHashtag(hashtagData));
    };
    ...
    useEffect(() => {
        if (status === Status.ERROR && error) {
            ...
            dispatch(hashtagActions.resetStatus());
        }
    }, [status, error]);
    useEffect(() => {
        ...
        if (isCreated || isDeleted) {
            ...
            dispatch(hashtagActions.resetIsCreated());
            dispatch(hashtagActions.resetIsDeleted());
            stopLoader(setIsLoaderOpened);
        }
    }, [isCreated, isDeleted]);
    useEffect(() => {
    ...
    useEffect(() => {
        if (hashtags && hashtags.length) {
            const formattedData = hashtags.map((hashtag) => ({
                id: hashtag.id,
                name: hashtag.name,
                object_count: hashtag.object_count,
            }));
            tabulator.current
    ...
    return (
        <>
            <div className="flex items-center mt-8 intro-y">
                <h2 className="mr-auto text-lg font-medium">Хэштеги</h2>
